    ...
    class MailLib {
        ...
        private function init() {
            ...
            try {
                ...
                $this->mail->FromName = 'no-reply@somedomain.nl';
                // $this->mail->SMTPAutoTLS = false;

                $this->mail->isSMTP();                                          // Set mailer to use SMTP
                $this->mail->Host       = 'smtp.yourhost.email';                // Specify main and backup SMTP servers
                $this->mail->SMTPAuth   = true;                                 // Enable SMTP authentication
                $this->mail->Username   = 'no-reply@domain.nl';                 // SMTP username
                $this->mail->From       = 'no-reply@domain.nl';
                $this->mail->Password   = 'your smpt password';                 // SMTP password
                $this->mail->SMTPSecure = 'ssl';                                // Enable TLS encryption, `ssl` also accepted
                $this->mail->Port       = 465;                                  // 465 is most common for ssl
        ...
        public function regularMail($to, $subject, $message, $reply=NULL) {
            try {
                $this->mail->addAddress($to, $name);               // Name is optional
